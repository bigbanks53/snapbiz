...
import { EASE } from './consultantsData'

const MotionLink = motion.create(Link)

export default function ConsultantsClosing() {
  ...
  return (
    <section
      id="consultants-closing"
      aria-labelledby="consultants-closing-heading"
    ...
    >
      ...
      <div className="relative mx-auto max-w-3xl px-5 py-24 text-center sm:px-8 sm:py-32 lg:py-36">
        ...
        >
          Consultant access is planned, not built. There is no sign-up list yet, so the honest
          answer for now is that SnapBiz will update this page when the feature is ready.
        </motion.p>
        ...
        <motion.div {...rise(0.24)} className="mt-10 flex flex-col items-center gap-4">
          <motion.button
            ...
            aria-label="Notify me when the consultants feature launches (preview only — notifications are not open yet)"
          >
        ...
        <motion.div {...rise(0.32)} className="mt-12 border-t border-ivory/10 pt-8">
          ...
          >
            See where Consultants sits on the path
            <ArrowRight
              size={16}
              strokeWidth={2.25}
              aria-hidden="true"
          ...
          <p className="mt-4 text-[11px] font-medium text-ivory/40 sm:text-xs">
            Consultants is stage 05 of six — marked{' '}
            <span className="text-ivory/60">Coming soon</span> there too.
          </p>
        </motion.div>
      </div>
    </section>
